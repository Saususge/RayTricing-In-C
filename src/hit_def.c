...
#define K_DIFFUSE 10.0f
#define K_SPECULAR 10.0f
...
t_vec3	get_color(const t_object *obj, const t_intersect *record)
{
	t_vec3	ambient;
	t_vec3	diffuse;
	t_vec3	specular;
	...
	float	point_to_light_dist;

	ambient = vec3_hadamard(vec3_mul(g()->amb_light.intensity, K_AMBIENT), obj->color);
	diffuse = (t_vec3){0, 0, 0};
	specular = (t_vec3){0, 0, 0};
	i = 0;
	while (i < g()->light_count)
	{
		hit_point_world = mat_mul_vec4(&record->object->m_inv, record->hit_point_local);
		light_v_world = vec4_sub(g()->lights[i].position, hit_point_world);
		dist_sq_world = vec4_dot(light_v_world, light_v_world);
		light_v_world = vec4_mul(light_v_world, 1.0f / dist_sq_world);
		...
		{
			t_vec4	light_v_local = vec4_sub(obj->local_light_pos[i], record->hit_point_local);
			t_vec4	normal_local = obj->ops->get_normal(obj, record->hit_point_local);
			attenuation = 1.0f / (1.0f + dist_sq_world);
			...
			diffuse = vec3_add(diffuse, vec3_hadamard(diffusion_light_intensity, obj->color));
...
int	hit_ray(const t_ray *ray_world, t_interval bound_world, t_intersect *intersect_record, t_vec3 *color)
{
	...
	*color = get_color(intersect_record->object, intersect_record);
	return (hit_anything);
}
